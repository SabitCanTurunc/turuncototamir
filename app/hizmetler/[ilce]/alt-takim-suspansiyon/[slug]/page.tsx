...
export async function generateStaticParams() {
    ...
    for (const district of HATAY_DISTRICTS) {
        for (const service of ALT_TAKIM_DETAILS) {
            params.push({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const service = ALT_TAKIM_DETAILS.find(s => s.slug === resolvedParams.slug);
    ...
    return {
        title,
        description,
        alternates: { canonical: `/hizmetler/${resolvedParams.ilce}/alt-takim-suspansiyon/${resolvedParams.slug}` },
    };
}
...
export default async function ServiceDetailPage({ params }: Props) {
    ...
    const otherServices = ALT_TAKIM_DETAILS.filter(s => s.slug !== service.slug);
                    ...
                    <div className="grid lg:grid-cols-3 gap-12">
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            ...
                            <div>
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-xs font-bold uppercase tracking-wider mb-6">
                                    <Wrench className="w-3 h-3" /> {districtName} Ön Takım ve Süspansiyon
                                </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="bg-slate-900/50 rounded-2xl p-8 border border-slate-800 hover:border-red-500/30 transition-colors">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h3 className="text-xl font-bold text-white">Bozulma Nedenleri</h3>
                                    </div>
                            ...
                            <div className="bg-blue-900/10 rounded-2xl p-8 border border-blue-500/20 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                                ...
                                <div>
                                    <h3 className="text-lg font-bold text-white mb-2">Değişim Kilometresi</h3>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="hidden lg:block">
                                <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Diğer Ön Takım İşlemleri</h3>
                                <div className="space-y-2">
                                    {otherServices.map((s) => (
                                        <Link
                                            key={s.slug}
                                            href={`/hizmetler/${resolvedParams.ilce}/alt-takim-suspansiyon/${s.slug}`}
