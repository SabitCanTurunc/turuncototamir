...
import { ELEKTRONIK_DETAILS } from '@/app/data/elektronik-data';
...
export async function generateStaticParams() {
    ...
    for (const district of HATAY_DISTRICTS) {
        for (const service of ELEKTRONIK_DETAILS) {
            params.push({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const service = ELEKTRONIK_DETAILS.find(s => s.slug === resolvedParams.slug);
    ...
    return {
        title,
        description,
        alternates: { canonical: `/hizmetler/${resolvedParams.ilce}/elektronik/${resolvedParams.slug}` },
    };
}
...
export default async function ServiceDetailPage({ params }: Props) {
    ...
    const otherServices = ELEKTRONIK_DETAILS.filter(s => s.slug !== service.slug);
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            ...
                            <div>
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-xs font-bold uppercase tracking-wider mb-6">
                                    <Wrench className="w-3 h-3" /> {districtName} Oto Elektronik Servisi
                                </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="bg-slate-900/50 rounded-2xl p-8 border border-slate-800 hover:border-red-500/30 transition-colors">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h3 className="text-xl font-bold text-white">Neden Arızalanır?</h3>
                                    </div>
                                ...
                                <div className="bg-slate-900/50 rounded-2xl p-8 border border-slate-800 hover:border-orange-500/30 transition-colors">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h3 className="text-xl font-bold text-white">Arıza Belirtileri</h3>
                                    </div>
                                    <ul className="space-y-4">
                            ...
                            <div className="bg-blue-900/10 rounded-2xl p-8 border border-blue-500/20 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                                ...
                                <div>
                                    <h3 className="text-lg font-bold text-white mb-2">Bakım ve Onarım Sıklığı</h3>
                        ...
                        <div className="space-y-8">
                            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
                                ...
                                <p className="text-slate-400 text-sm mb-6">
                                    {districtName} şubemizde uzman ekibimizle aracınızın {service.title.toLowerCase()} işlemlerini garantili olarak yapıyoruz.
                                </p>
                                <div className="space-y-4">
                                    <a href={`tel:${APP_CONFIG.phoneRaw}`} className="flex items-center justify-center gap-3 bg-white text-slate-950 px-6 py-4 rounded-xl font-bold hover:bg-orange-500 hover:text-white transition-all w-full">
                                        <Phone className="w-5 h-5" /> Hemen Ara
                                    </a>
                                    <a href={`https://wa.me/${APP_CONFIG.whatsapp}?text=Merhaba, ${districtName} bölgesi için ${service.title} hakkında bilgi almak istiyorum.`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 bg-[#25D366] text-white px-6 py-4 rounded-xl font-bold hover:bg-[#128C7E] transition-all w-full">
                            ...
                            <div className="hidden lg:block">
                                <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Diğer Elektronik İşlemleri</h3>
                                <div className="space-y-2">
                                    {otherServices.map((s) => (
                                        <Link
                                            key={s.slug}
                                            href={`/hizmetler/${resolvedParams.ilce}/elektronik/${s.slug}`}
