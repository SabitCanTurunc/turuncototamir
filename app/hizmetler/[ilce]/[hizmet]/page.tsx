...
import { MOTOR_MEKANIK_DETAILS } from '@/app/data/motor-mekanik-data';
import { PERIYODIK_BAKIM_DETAILS } from '@/app/data/periyodik-bakim-data';
import { FREN_DETAILS } from '@/app/data/fren-data';
import { ELEKTRONIK_DETAILS } from '@/app/data/elektronik-data';
import { SANZIMAN_DETAILS } from '@/app/data/sanziman-tamiri-data';
import { ALT_TAKIM_DETAILS } from '@/app/data/alt-takim-suspansiyon-data';
            ...
            <div className="relative">
              ...
              <div className="relative bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 sm:p-10 shadow-2xl">
                ...
                <div className="grid gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {isMotorMekanik ? (
                    MOTOR_MEKANIK_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/motor-mekanik/${sub.slug}`}
                        className="flex items-start gap-3 group hover:bg-slate-800/50 p-2 rounded-lg transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
                        <div className="flex-1 border-b border-dashed border-slate-800 pb-2 group-hover:border-transparent">
                          <span className="text-slate-300 group-hover:text-white transition-colors text-base font-medium block">
                            {sub.title}
                          </span>
                          <span className="text-xs text-slate-500 group-hover:text-orange-400 transition-colors">
                            Detaylı İncele →
                          </span>
                        </div>
                      </Link>
                    ...
                    PERIYODIK_BAKIM_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/periyodik-bakim/${sub.slug}`}
                        className="flex items-start gap-3 group hover:bg-slate-800/50 p-2 rounded-lg transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
                        <div className="flex-1 border-b border-dashed border-slate-800 pb-2 group-hover:border-transparent">
                    ...
                    FREN_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/fren/${sub.slug}`}
                        className="flex items-start gap-3 group hover:bg-slate-800/50 p-2 rounded-lg transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
                        <div className="flex-1 border-b border-dashed border-slate-800 pb-2 group-hover:border-transparent">
                  ...
                  ) : (serviceItem.slug as string) === 'fren' ? (
                    FREN_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/fren/${sub.slug}`}
                  ...
                  ) : (serviceItem.slug as string) === 'elektronik' ? (
                    ELEKTRONIK_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/elektronik/${sub.slug}`}
                        className="flex items-start gap-3 group hover:bg-slate-800/50 p-2 rounded-lg transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
                        <div className="flex-1 border-b border-dashed border-slate-800 pb-2 group-hover:border-transparent">
                  ...
                  ) : (serviceItem.slug as string) === 'sanziman-tamiri' ? (
                    SANZIMAN_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/sanziman-tamiri/${sub.slug}`}
                        className="flex items-start gap-3 group hover:bg-slate-800/50 p-2 rounded-lg transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
                        <div className="flex-1 border-b border-dashed border-slate-800 pb-2 group-hover:border-transparent">
                  ...
                  ) : (serviceItem.slug as string) === 'alt-takim-suspansiyon' ? (
                    ALT_TAKIM_DETAILS.map((sub, idx) => (
                      <Link
                        key={idx}
                        href={`/hizmetler/${resolvedParams.ilce}/alt-takim-suspansiyon/${sub.slug}`}
                        className="flex items-start gap-3 group hover:bg-slate-800/50 p-2 rounded-lg transition-all"
                      >
                        <CheckCircle2 className="w-5 h-5 text-orange-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
                        <div className="flex-1 border-b border-dashed border-slate-800 pb-2 group-hover:border-transparent">
