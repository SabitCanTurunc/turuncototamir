...
export async function generateStaticParams() {
    ...
    for (const district of HATAY_DISTRICTS) {
        for (const service of FREN_DETAILS) {
            params.push({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const service = FREN_DETAILS.find(s => s.slug === resolvedParams.slug);
    ...
    return {
        title,
        description,
        alternates: { canonical: `/hizmetler/${resolvedParams.ilce}/fren/${resolvedParams.slug}` },
    };
}
...
export default async function ServiceDetailPage({ params }: Props) {
    ...
    const otherServices = FREN_DETAILS.filter(s => s.slug !== service.slug);
                    ...
                    <div className="grid lg:grid-cols-3 gap-12">
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            ...
                            <div>
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-xs font-bold uppercase tracking-wider mb-6">
                                    <Wrench className="w-3 h-3" /> {districtName} Fren Bakım Servisi
                                </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="hidden lg:block">
                                <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Diğer Fren İşlemleri</h3>
                                <div className="space-y-2">
                                    {otherServices.map((s) => (
                                        <Link
                                            key={s.slug}
                                            href={`/hizmetler/${resolvedParams.ilce}/fren/${s.slug}`}
