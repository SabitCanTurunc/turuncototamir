...
import { HATAY_DISTRICTS, APP_CONFIG } from '@/app/constants';
import { MOTOR_MEKANIK_DETAILS } from '@/app/data/motor-mekanik-data';
import { PageLayout } from '@/app/components/layout/PageLayout';
import { Phone, MessageCircle, ArrowLeft, Wrench, AlertTriangle, Clock, Activity, CheckCircle2 } from 'lucide-react';

// Static Params Generation
export async function generateStaticParams() {
    const params = [];
    for (const district of HATAY_DISTRICTS) {
        for (const service of MOTOR_MEKANIK_DETAILS) {
            params.push({
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const service = MOTOR_MEKANIK_DETAILS.find(s => s.slug === resolvedParams.slug);

    if (!district || !service) return { title: 'Hizmet Bulunamadı' };

    return {
        title: `${district} ${service.title} - Turunç Oto Tamir`,
        description: `${district} bölgesinde ${service.title} hizmeti. ${service.shortDescription} Belirtileri: ${service.symptoms.slice(0, 2).join(', ')}.`,
        alternates: { canonical: `/hizmetler/${resolvedParams.ilce}/motor-mekanik/${resolvedParams.slug}` },
    };
}
...
export default async function ServiceDetailPage({ params }: Props) {
    ...
    const otherServices = MOTOR_MEKANIK_DETAILS.filter(s => s.slug !== service.slug);
                    ...
                    <div className="grid lg:grid-cols-3 gap-12">
                        ...
                        <div className="lg:col-span-2 space-y-12">
                            ...
                            <div>
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-xs font-bold uppercase tracking-wider mb-6">
                                    <Wrench className="w-3 h-3" /> {districtName} Motor Mekanik Servisi
                                </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                ...
                                <div className="bg-slate-900/50 rounded-2xl p-8 border border-slate-800 hover:border-red-500/30 transition-colors">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h3 className="text-xl font-bold text-white">Neden Bozulur?</h3>
                                    </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="hidden lg:block">
                                <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-4">Diğer Motor İşlemleri</h3>
                                <div className="space-y-2">
                                    {otherServices.map((s) => (
                                        <Link
                                            key={s.slug}
                                            href={`/hizmetler/${resolvedParams.ilce}/motor-mekanik/${s.slug}`}
